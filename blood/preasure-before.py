...
data_y = np.loadtxt('before.txt')
data_x = []
dt = 30 / np.size(data_y)
time = dt
# print(dt)
...
for i in range(0, np.size(data_y)):
    data_x.append(time)
    time += dt
    if time >= 10 and flag_s is False:
        sist = i
        flag_s = True
    if time >= 15.5 and flag_d is False:
        dias = i
        flag_d = True
    if time >= 19:
        break
...
ax.plot(data_x, data_limited, color='blue', linewidth=2, label='p(t)')
...
title_text = 'Зависимость значения артериального давления от времени в спокойном состоянии'
