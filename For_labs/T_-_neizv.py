...
def speedOfSound(temperature, h2oX, co2Max):
    ...
    R = 8.31
    Mi = [0.028, 0.032, 0.04, 0.044] #N2, O2, Ar, CO2 соотв
    Ci = [0.7809, 0.2095, 0.0093, 0.03]
    Cpi = [5, 5, 3, 6]
    M = 0
    C_H2O = h2oX * 3360 / 101375
    Ci[2] = Ci[2] * (1 - C_H2O)
    Ci[0] = Ci[0] * (1 - C_H2O)
    ...
    for i in range(4):
        M += Mi[i] * Ci[i]
        adiobata_up += Mi[i] * Ci[i] * R * (Cpi[i] + 2)
        adiobata_down += Mi[i] * Ci[i] * R * Cpi[i]
        #Деление на 2 убираем из формулы Cpi(m), т.к оно сократится
    co2X = []
    adiobata = adiobata_up / adiobata_down
#    print(adiobata)
#    print(M)
    soundSpeed = []
    soundSpeed.append((adiobata * R * (temperature+273) / M)**0.5)
    co2X.append((adiobata * R * temperature / M)**0.5)
...
def analytics():
    ...
    def foo(x, a, b, c):    return a * x**2 + b*x + c
...
#обработаем
mas_1 = for_find_t(len(data0), data0)
y1 = mas_1[1]
x_max_1 = mas_1[0] #1746
mas_2 = for_find_t(len(data1), data1)
y2 = mas_2[1]
x_max_2 = mas_2[0] #250
...
#обработаем
mas_1 = for_find_t(len(data0_1), data0_1)
y1 = mas_1[1]
x_max_1 = mas_1[0] #1746
mas_2 = for_find_t(len(data1_1), data1_1)
y2 = mas_2[1]
x_max_2 = mas_2[0] #250
...
T2 = x_max_1 - x_max_2 #углекислый газ
...
for i in range(5001):
    ...
    y1.append(arr1[i][1][0])
for i in range(5001):
    arr2.append(speedOfSound(t, 1, x[i]))
    y2.append(arr2[i][1][0])
